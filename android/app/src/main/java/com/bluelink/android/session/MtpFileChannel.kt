...
internal class MtpFileChannel(private val context: Context, private val scope: CoroutineScope,
    ...
    suspend fun receive(payload: ByteArray) {
        ...
        when (packet.getString("op")) {
            "start" -> { entry.active = true; entry.start.complete(Unit) }
            ...
            "blob" -> {
                ...
                entry.worker = scope.launch { receiveBlob(entry, packet) }
            }
        }
    }
    ...
    suspend fun runOutgoing(offer: FileOffer, progress: TransferPauseController, body: suspend () -> Unit, onQueued: () -> Unit = {}) {
        ...
        try {
            progress.item?.let { progress.report(it.copy(status = TransferStatus.QUEUED, completedBytes = 0)) }
    ...
    suspend fun sendBlob(id: UUID, input: InputStream) {
        ...
        try {
            val uri = entry.spool.createBlob(name)
