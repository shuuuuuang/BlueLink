...
public enum TransferStatus { Offered, Queued, Transferring, Paused, Resuming, Verifying, Committing, Completed, Rejected, Failed, Canceled }
...
public sealed record ConversationSummary(string PeerId, string PeerName, PeerPlatform Platform,
    ...
    int UnreadCount, DateTimeOffset LastActivityAt)
{
    public string AvailabilityText => Availability switch
    {
        DeviceAvailability.Connected => "已连接 · 端到端加密",
        DeviceAvailability.Offline => $"离线 · {LastActivityAt.ToLocalTime():MM-dd HH:mm}",
        _ => "附近可连接"
    };
    ...
    public string LastSeenText => Availability == DeviceAvailability.Offline
        ? $"最后在线 {LastActivityAt.ToLocalTime():MM-dd HH:mm}"
        : AvailabilityText;
    ...
    long CompletedBytes = 0, double BytesPerSecond = 0)
{
    ...
    public string StateText => IsMissing ? "文件已移动或删除" : State switch
    {
        "Completed" => "已完成",
        "Transferring" => "传输中",
        "Verifying" => "正在校验",
    ...
    public bool IsTransferActive => State is "Offered" or "Queued" or "Transferring" or "Paused" or
    ...
    public string ProgressText => $"{Format(CompletedBytes)} / {Format(Size)}" +
        (BytesPerSecond > 0 && IsTransferActive ? $" · {Format((long)BytesPerSecond)}/s" : "");
...
{
    public string Sender => Outgoing ? "我" : "对方";
    ...
    public string GroupTimeText
    {
        get
        {
            ...
            if (date == today.AddDays(-1)) return $"昨天 {time}";
            ...
            if (date >= startOfWeek)
            {
                var weekday = date.DayOfWeek switch
                {
                    ...
                    DayOfWeek.Friday => "周五", DayOfWeek.Saturday => "周六", _ => "周日"
                };
                return $"{weekday} {time}";
            }
            return date.Year == today.Year ? value.ToString("M月d日 HH:mm") : value.ToString("yyyy年M月d日 HH:mm");
        }
    }
    public string StatusText => Status switch
    {
        MessageStatus.LocalQueued => "等待重连",
...
public sealed class TransferItem : INotifyPropertyChanged
{
    ...
    public TransferStatus Status
    {
        ...
        set
        {
            ...
            Changed(nameof(CanOpen)); Changed(nameof(CanDelete)); Changed(nameof(StatusText));
        }
    }
    public double Progress => TotalBytes == 0 ? 1 : Math.Clamp((double)CompletedBytes / TotalBytes, 0, 1);
    public string Direction => Outgoing ? "发送" : "接收";
    public double BytesPerSecond => _bytesPerSecond;
    public string SpeedText => _bytesPerSecond > 0 ? $"{Format((long)_bytesPerSecond)}/s" : "正在估算速度";
    public string RemainingText => _bytesPerSecond > 0 && TotalBytes > CompletedBytes
        ? $"约 {TimeSpan.FromSeconds((TotalBytes - CompletedBytes) / _bytesPerSecond):mm\\:ss}"
        : "";
    ...
    public string StatusText => Status switch
    {
        TransferStatus.Offered => Outgoing ? "等待对端接受" : "已接受",
        ...
        TransferStatus.Committing => "正在落盘",
